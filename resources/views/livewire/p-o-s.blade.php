<div class="flex h-screen bg-gray-100 dark:bg-neutral-900 font-sans antialiased text-gray-800 dark:text-gray-100">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse ($this->filteredItems as $item)
                    <div class="bg-white dark:bg-neutral-800 rounded-2xl shadow-lg overflow-hidden 
                        ...
                        <div class="p-4">
                            <!-- Image -->
                            <div class="w-full h-32 rounded-lg mb-3 overflow-hidden bg-gray-100 dark:bg-neutral-700">
                                <!-- {{-- Stock badge --}} -->
                        ...
                        <button
                            ...
                            class="w-full py-3 rounded-lg bg-amber-600 text-white font-semibold
                                hover:bg-amber-700
 ...
 {{-- Right panel --}}
    <div
                ...
                <div class="flex items-center justify-between p-4 mb-4 bg-gray-50 dark:bg-neutral-700 rounded-xl shadow-sm">
                    <div class="flex-1">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            $ {{ number_format($cartItem['price'], 2) }} / {{ number_format($cartItem['price_khr'] ?? 0) }}៛ each
                        </p>
            ...
            <div class="mt-6 pt-6 border-t border-gray-200 dark:border-neutral-700">
                <div class="flex justify-between items-center mb-2">
                    ...
                    <span class="font-medium text-gray-800 dark:text-gray-100">
                        $ {{ number_format($this->subtotal, 2) }} / {{ number_format($this->subtotalKhr ?? 0) }}៛
                    </span>
                </div>
                ...
                <div class="flex justify-between items-center mb-2">
                    ...
                    <span class="font-medium text-gray-800 dark:text-gray-100">$
                        {{ number_format($this->tax, 2) }} / {{ number_format($this->taxKhr ?? 0) }}៛</span>
                </div>

                <div class="flex justify-between items-center mb-2">
                    <span class="text-sm text-gray-600 dark:text-gray-400">Total before discount:</span>
                    <span class="font-medium text-gray-800 dark:text-gray-100">$
                        {{ number_format($this->totalBeforeDiscount, 2) }} / {{ number_format($this->totalBeforeDiscountKhr ?? 0) }}៛</span>
                </div>
                ...
                <div
                    ...
                    <span>$ {{ number_format($this->total, 2) }} / {{ number_format($this->totalKhr ?? 0) }}៛</span>
                </div>
                <div
                    class="flex justify-between items-center text-lg font-bold mt-2 border-t border-gray-200 dark:border-neutral-700 pt-2">
                    <span>Change Given:</span>
                    <span>$ {{ number_format($this->change, 2) }} / {{ number_format($this->changeKhr ?? 0) }}៛</span>
                </div>
            </div>
        </div>
        ...
        @else
            <div class="mt-6 p-4 text-center bg-yellow-100 dark:bg-yellow-900 rounded-xl">
                <p class="text-yellow-700 dark:text-yellow-300 font-semibold">
                    Manager role is view-only. Checkout disabled.
                </p>
            </div>
        @endif
    </div>
